...
domains = ['mail.ru', 'gmail.com', 'example.com']


@pytest.fixture(params=domains)
def email(request) -> str:
    return fake.email(request.param)
...
def test_get_user_me(function_user: UserFixture, private_users_client: PrivateUsersClient):
    ...
    assert_status_code(response.status_code, HTTPStatus.OK)
    assert_get_user_response(create_user_response=function_user.response.user, get_user_response=response_data.user)
